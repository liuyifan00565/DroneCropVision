...
from tasks.detect.seedling import config as config
...
def count_files_in_directory(import_img_sampling_exit, import_file_list, allowed_types):
    ...
    for file_dict in import_file_list:
        if is_file_type_within_range(file_dict, allowed_types):
            list_info = list()
            list_info.append(True)
            list_info.append(file_dict)
            list_info.append(0)
            list_info.append(0)
            list_info.append(0)
            list_info.append(0)
...
def get_first_sampling_result(import_file_choose_list, sampling_count, import_img_sampling_exit, download_path,
                              ...
                              predict_path, allowed_types, cut_size, sampling_box_color, area):
    ...
    for import_img in random_subset:
        ...
        # 切图
        w, h, size_list_now = utils.split_image(import_img, file_path, cut_path, cut_size)
        import_img_sampling_exit[import_img][2] = w
        import_img_sampling_exit[import_img][3] = h
        import_img_sampling_exit[import_img][4] = area
        size_list |= size_list_now
    for size in size_list:
        print(f"此次处理的图片尺寸为：{size}")
        # 获取预测样本总数
        random_subset_cut = list()
        for f in os.listdir(cut_path):
            if os.path.isfile(os.path.join(cut_path, f)) and utils.is_file_type_size_within_range(os.path.join(cut_path, f),
                                                                                       allowed_types, size):
                random_subset_cut.append(f)
        # 获取样本需要预测的次数
        for_cut_count = math.ceil(len(random_subset_cut) / predict_count)
        ...
        for i in range(for_cut_count):
            ...
            for j in range(i * predict_count, final_count):
                ...
                import_file_choose_list.append(import_img_sampling_exit[key][1])
                image_local = Image.open(file_path)
                w, h = image_local.size
                area_cut = ((w * h * import_img_sampling_exit[key][4]) /
                            (import_img_sampling_exit[key][2] * import_img_sampling_exit[key][3]))
                area_list.append(area_cut)
            print("单次调用预测方法时的文件列表：", predict_list)
            print("单次调用预测方法时的名字列表：", file_name_list)
            print("单次调用预测方法时的区域大小列表：", area_list)
            # 获取图片预测结果
            list_counts, count_areas, class_ids, confidence, rects, import_img_sampling_exit = (
                predict.get_predict_counts_for_sampling(size, predict_list, file_name_list, predict_path, False, area_list, import_img_sampling_exit, cut_size, sampling_box_color, cut_path))
...
def get_result_sampling_t(t_num, import_img_sampling_exit):
    total_count = 0
    sampling_count = 0
    list_sampling = list()
    for key in import_img_sampling_exit:
        if not import_img_sampling_exit[key][0]:
            total_count += import_img_sampling_exit[key][5]
            sampling_count += 1
            list_sampling.append(import_img_sampling_exit[key][5])
    # 计算平均值
    average = total_count / sampling_count
    print(f"平均值为：{average}")
    # 计算标准差  n-1   s
...
def get_sampling_result(import_file_choose_list, import_file_list, import_img_sampling_exit, download_path, n,
                        ...
                        cut_path,
                        predict_count, predict_path, allowed_types, cut_size, sampling_box_color, area):
    ...
    average, confidence_left, confidence_right, confidence_percent, variance = get_result_sampling_t(t_num, import_img_sampling_exit)
    print(
...
def save_return_img(import_img_sampling_exit, download_path, final_path, predict_path):
    exit_set = set()
    for key in import_img_sampling_exit:
        if not import_img_sampling_exit[key][0]:
            exit_set.add(key)
    for file_name in exit_set:
        counts = import_img_sampling_exit[file_name][5]
        area = import_img_sampling_exit[file_name][4]
        text = f"苗数: {counts}\n单位面积苗数: {counts / area}"
...
def get_result(import_file_list, download_path, n, confidence, expected_percent, cut_size, cut_path, predict_count,
    ...
    average, confidence_left, confidence_right, confidence_percent, import_img_sampling_exit, sampling_n, import_file_choose_list, class_ids_list, confidence_list, rects_list = get_sampling_result(
        ...
        expected_percent, cut_path, predict_count, predict_path, allowed_types, cut_size, sampling_box_color, area)
    # 判断是否需要输出最终的拼图
    if if_return_img:
        # 拼图
        save_return_img(import_img_sampling_exit, download_path, final_path, predict_path)
    import_file_choose_list = list()
    class_ids_list = list()
    confidence_list = list()
    rects_list = list()
    for key in import_img_sampling_exit:
        if not import_img_sampling_exit[key][0]:
            import_file_choose_list.append(import_img_sampling_exit[key][1])
            class_ids_list.append(import_img_sampling_exit[key][6])
            confidence_list.append(import_img_sampling_exit[key][8])
            rects_list.append(import_img_sampling_exit[key][7])
